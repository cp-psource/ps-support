				...
				<div class="support-system-reply-poster">
					<h3>
						...
						<?php if ( incsub_support_is_staff( incsub_support_get_the_poster_id() ) ): ?>
							<span class="label"><?php _e( 'Mitarbeiter', INCSUB_SUPPORT_LANG_DOMAIN ); ?></span>
						<?php endif; ?>
					</h3>
				</div>
			</div>
					...
					<?php if ( incsub_support_reply_has_attachments() ): ?>
						<div class="support-system-reply-attachments">
							...
							<?php foreach ( incsub_support_get_the_reply_attachments() as $attachment_url ): ?>
								<li><a href="<?php echo esc_url( $attachment_url ); ?>" title="<?php printf( __( 'Download %s Datei', INCSUB_SUPPORT_LANG_DOMAIN ), $attachment_url ); ?>" ><?php echo basename( $attachment_url ); ?></a></li>
							<?php endforeach; ?>
							</ul>
						</div>
					<?php endif; ?>
