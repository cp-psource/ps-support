	...
	public function process_form() {
		if ( isset( $_POST['support-system-submit-ticket'] ) && incsub_support_current_user_can( 'insert_ticket' ) ) {
			...
			if ( ! wp_verify_nonce( $_POST['_wpnonce'], $action ) )
				wp_die( __( 'Sicherheitsüberprüfungsfehler', INCSUB_SUPPORT_LANG_DOMAIN ) );

			$subject = $_POST['support-system-ticket-subject'];
			if ( empty( $subject ) )
				wp_die( __( 'Bitte gib einen Betreff für das Ticket ein', INCSUB_SUPPORT_LANG_DOMAIN ) );

			$message = $_POST['support-system-ticket-message'];
			if ( empty( $message ) )
				wp_die( __( 'Bitte füge eine Nachricht für das Ticket ein', INCSUB_SUPPORT_LANG_DOMAIN ) );
	...
	public function render( $atts ) {
		...
		if ( ! incsub_support_current_user_can( 'insert_ticket' ) ) {
			if ( ! is_user_logged_in() )
				$message = sprintf( __( 'Du musst <a href="%s">angemeldet</a> sein, um ein neues Ticket zu erstellen', INCSUB_SUPPORT_LANG_DOMAIN ), wp_login_url( get_permalink() ) );
			else
				$message = __( 'Du hast nicht genügend Berechtigungen, um ein neues Ticket einzureichen', INCSUB_SUPPORT_LANG_DOMAIN );
		...
		if ( ! incsub_support()->query->is_single_ticket ) {
			?>
				<h2><?php _e( 'Sende ein neues Ticket', INCSUB_SUPPORT_LANG_DOMAIN ); ?></h2>
				<form method="post" id="support-system-ticket-form" action="#support-system-ticket-form-wrap" enctype="multipart/form-data">
					
					<input type="text" name="support-system-ticket-subject" value="" placeholder="<?php esc_attr_e( 'Betreff', INCSUB_SUPPORT_LANG_DOMAIN ); ?>"/>
					<br/>
					...
					<?php if ( $blog_field && is_multisite() ): ?>
						<label for="support-system-ticket-blog">
							<?php _e( 'Meldest Du ein Ticket für eine bestimmte Site?', INCSUB_SUPPORT_LANG_DOMAIN ); ?>
					...
					<input type="submit" name="support-system-submit-ticket" class="button small" value="<?php esc_attr_e( 'Ticket übermitteln', INCSUB_SUPPORT_LANG_DOMAIN ); ?>" />
