...
function incsub_support_send_user_new_ticket_mail( $ticket_id ) {
	...
	wp_mail( $ticket_creator->data->user_email, __( "Ticket eingereicht: ", INCSUB_SUPPORT_LANG_DOMAIN ) . $ticket->title, $mail_content, $headers );
}
...
function incsub_support_send_admin_new_ticket_mail( $ticket_id ) {
	...
	if ( ! $poster )
		$poster_name = __( 'Unbekannter Benutzer', INCSUB_SUPPORT_LANG_DOMAIN );
	else
	...
	wp_mail( $user->user_email, __( "Neues Support Ticket: ", INCSUB_SUPPORT_LANG_DOMAIN ) . $ticket->title, $mail_content, $headers );
}
...
function incsub_support_send_user_reply_mail( $ticket, $reply ) {
	...
	wp_mail( $ticket_creator->user_email, __( "Benachrichtigung über Ticketantwort: ", INCSUB_SUPPORT_LANG_DOMAIN ) . $reply->subject, $mail_content, $headers );
}
...
function incsub_support_send_admin_reply_mail( $admin_user, $ticket, $reply ) {
	...
	wp_mail( $admin_user->user_email, __( "Benachrichtigung über Ticketantwort: ", INCSUB_SUPPORT_LANG_DOMAIN ) . $reply->subject, $mail_content, $headers );
}
...
function incsub_support_send_user_closed_mail( $ticket_id ) {
	...
	wp_mail( $creator->user_email, __( "Benachrichtigung über geschlossenes Ticket: ", INCSUB_SUPPORT_LANG_DOMAIN ) . $ticket->title, $mail_content, $headers );
}


function incsub_support_get_support_fetch_imap_message() {
	if ( get_site_option( 'incsub_support_fetch_imap', 'disabled' ) == 'enabled' )
		$support_fetch_imap = __( "***  NICHT UNTERHALB DIESER LINIE SCHREIBEN  ***", INCSUB_SUPPORT_LANG_DOMAIN );
	else
		$support_fetch_imap = __("***  ANTWORTE NICHT AUF DIESE E-MAIL  ***", INCSUB_SUPPORT_LANG_DOMAIN );
...
Danke", INCSUB_SUPPORT_LANG_DOMAIN );
