...
class Incsub_Support_Parent_Support_Menu extends Incsub_Support_Admin_Menu {
	...
	public function on_load() {
		...
		if ( $action !== 'edit' )
			add_screen_option( 'per_page', array( 'label' => __( 'Tickets pro Seite', 'incsub-support' ), 'default' => 20, 'option' => 'incsub_support_tickets_per_page' ) );
		...
		if ( ! empty( $_POST['submit-ticket-details'] ) ) {
			...
			if ( ! $ticket )
				wp_die( __( 'Das Ticket existiert nicht', INCSUB_SUPPORT_LANG_DOMAIN ) );
		...
		if ( isset( $_POST['submit-ticket-reply'] ) ) {
			...
			if ( empty( $message ) )
				add_settings_error( 'support_system_submit_reply', 'empty-message', __( 'Nachricht kann nicht leer sein', INCSUB_SUPPORT_LANG_DOMAIN ) );
			else
	...
	protected function render_inner_page_details() {
		...
		add_meta_box( 'support-system-ticket-details', __( 'Ticketdetails', INCSUB_SUPPORT_LANG_DOMAIN ), array( $this, 'render_details_metabox' ), $screen->id, 'normal' );
		add_meta_box( 'support-system-ticket-history', __( 'Ticketverlauf', INCSUB_SUPPORT_LANG_DOMAIN ), array( $this, 'render_history_metabox' ), $screen->id, 'normal' );
		
		if ( $user_can_update_ticket || $user_can_close_ticket )
			add_meta_box( 'support-system-ticket-update', __( 'Ticket aktualisieren', INCSUB_SUPPORT_LANG_DOMAIN ), array( $this, 'render_update_metabox' ), $screen->id, 'side' );
		...
		?>
			...
			<?php if ( $ticket->is_closed() ): ?>
				<div class="error"><p><?php _e( 'Dieses Ticket wurde geschlossen', INCSUB_SUPPORT_LANG_DOMAIN ); ?></p></div>
			<?php endif; ?>
			<div id="poststuff">
	...
	public function render_details_metabox( $ticket_id ) {
		...
		// Submitted from
		$submitted_blog_link = __( 'Unbekannt', INCSUB_SUPPORT_LANG_DOMAIN );
		if ( is_multisite() ) {
	...
	public function render_update_metabox( $ticket_id ) {
        ...
        if ( incsub_support_current_user_can( 'update_ticket' ) ) {
			...
			$super_admins_dropdown = ! $ticket->get_staff_login() ? __( 'Noch nicht zugewiesen', INCSUB_SUPPORT_LANG_DOMAIN ) : $ticket->get_staff_login();
		...
		$fields = array(
 			'ticket-staff' => array( 
 				'label' => '<label for="super-admins">' . __( 'Personalvertreter', INCSUB_SUPPORT_LANG_DOMAIN ) . '</label>',
 				'content' => $super_admins_dropdown
 			),
 			'ticket-priority' => array( 
 				'label' => '<label for="ticket-priority">' . __( 'Priorität', INCSUB_SUPPORT_LANG_DOMAIN ) . '</label>',
 				'content' => $priorities_dropdown
 			),
			'ticket-category' => array( 
				'label' => '<label for="ticket-cat">' . __( 'Kategorie', INCSUB_SUPPORT_LANG_DOMAIN ) . '</label>',
		...
		if ( incsub_support_current_user_can( 'close_ticket', $ticket->ticket_id ) ) {
			$fields['ticket-closed'] = array(
				'label' => '<label for="close-ticket-checkbox">' . __( 'Ticket geschlossen', INCSUB_SUPPORT_LANG_DOMAIN ) . '</label>',
