...
class Incsub_Support_Admin_Support_Menu extends Incsub_Support_Parent_Support_Menu {
	...
	public function add_menu() {
		...
		if ( incsub_support_current_user_can( 'insert_ticket' ) ) {
			add_submenu_page( 
				$this->slug, 
				__( 'Neues Ticket erstellen', INCSUB_SUPPORT_LANG_DOMAIN ), 
				__( 'Neues Ticket erstellen', INCSUB_SUPPORT_LANG_DOMAIN ), 
				'read', 
	...
	public function add_new_ticket_link( $title ) {
		...
		return '<h2>'. $menu_title . ' <a href="' . esc_url( $add_new_link ) . '" class="add-new-h2">' . esc_html__( 'Neues Ticket erstellen', INCSUB_SUPPORT_LANG_DOMAIN ) . '</a></h2>';
	}

	public function add_new_ticket_title( $title ) {
		return '<h2>' . __( 'Neues Ticket erstellen', INCSUB_SUPPORT_LANG_DOMAIN ) . '</h2>';
	}
	...
	public function maybe_insert_new_ticket() {
		if ( isset( $_POST['submit-new-ticket'] ) ) {
			...
			if ( empty( $_POST['message-text'] ) )
				add_settings_error( 'support_system_submit_new_ticket', 'empty_message', __( 'Die Ticketnachricht darf nicht leer sein', INCSUB_SUPPORT_LANG_DOMAIN ) );
			else
				$args['message'] = wpautop( stripslashes_deep( $_POST['message-text'] ) );

			$title = strip_tags( stripslashes_deep( $_POST['subject'] ) );
			if ( empty( $title ) )
				add_settings_error( 'support_system_submit_new_ticket', 'empty_subject', __( 'Der Betreff des Tickets darf nicht leer sein', INCSUB_SUPPORT_LANG_DOMAIN ) );
			else
				$args['title'] = $title;

			$category = incsub_support_get_ticket_category( absint( $_POST['ticket-cat'] ) );
			if ( ! $category ) {
				add_settings_error( 'support_system_submit_new_ticket', 'wrong_category', __( 'Die ausgewählte Kategorie ist nicht gültig', INCSUB_SUPPORT_LANG_DOMAIN ) );
			}
			else {
